...
const scriptsMgr = function () {
    return gulp.src([
        'source/js/mgr/dbadmin.js',
        'source/js/mgr/misc/utils.js',
        'source/js/mgr/widgets/home.panel.js',
        'source/js/mgr/widgets/table.window.js',
        'source/js/mgr/widgets/tables.grid.js',
        'source/js/mgr/widgets/sql.panel.js',
        'source/js/mgr/widgets/data.grid.js',
        'source/js/mgr/sections/home.js'
    ])
        .pipe(concat('dbadmin.min.js'))
        .pipe(uglify())
        .pipe(header(banner + '\n', {pkg: pkg}))
        .pipe(gulp.dest('assets/components/dbadmin/js/mgr/'))
};
gulp.task('scripts', gulp.series(scriptsMgr));

const sassMgr = function () {
    return gulp.src([
        'source/sass/mgr/dbadmin.scss'
    ])
        ...
        .pipe(footer('\n' + banner, {pkg: pkg}))
        .pipe(gulp.dest('assets/components/dbadmin/css/mgr/'))
};
gulp.task('sass', gulp.series(sassMgr));

const imagesMgr = function () {
    return gulp.src('./source/img/**/*.+(png|jpg|gif|svg)', {encoding: false})
        .pipe(gulp.dest('assets/components/dbadmin/img/'));
};
gulp.task('images', gulp.series(imagesMgr));

const bumpCopyright = function () {
    return gulp.src([
        'core/components/dbadmin/model/dbadmin/dbadmin.class.php',
        'core/components/dbadmin/src/dbAdmin.php',
    ], {base: './'})
